...
function roundRating(rating) {
  ...
  const chaharhezar = [];
  const panjhezar = [];
  if (0 < rating && rating <= 1000) {
    for (let actor of actorFeature) {
      if (actor.rating <= 1000) {
        hezar.push(actor);
      }
    }
    return hezar;
    ...
    for (let actor of actorFeature) {
      if (actor.rating > 1000 && actor.rating <= 2000 ) {
        dohezar.push(actor);
      }
    }
    return dohezar;
  } else if (2000 < rating && rating <= 3000) {
    for (let actor of actorFeature) {
      if (actor.rating > 2000 && actor.rating <= 3000) {
        sehezar.push(actor);
      }
    }
    return sehezar;
  } else if (3000 < rating && rating <= 4000) {
    for (let actor of actorFeature) {
      if (actor.rating > 3000 && actor.rating <= 4000) {
        chaharhezar.push(actor);
      }
    }
    return chaharhezar;
  } else if(4000 < rating && rating <= 5000) {
    for (let actor of actorFeature) {
      if (actor.rating > 4000 && actor.rating <= 5000) {
        panjhezar.push(actor);
      }
    }
    return panjhezar;
  }
}
console.log(roundRating(2500));
